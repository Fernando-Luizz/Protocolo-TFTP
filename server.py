            ...
            while True:
                ...
                for tentativa in range(3):
                    ...
                    try:
                        ...
                        if (ack_info['opcode'] == OP_ACK
                                and ack_info['block_number'] == block_num):
                            break
                    except socket.timeout:
                        if tentativa == 2:
                            return  # Desiste após 3 tentativas
        ...
        with open(filename, "wb") as f:
            ...
            while True:
                try:
                    ...
                    if (data_info['opcode'] == OP_DATA
                            and data_info['block_number'] == expected_block):
                        f.write(data_info['data'])
                        transfer_socket.sendto(build_ack(expected_block), addr)
                        if len(data_info['data']) < BLOCK_SIZE:
                            break
...
def start_server():
    ...
    parser.add_argument(
        "-p", "--port", type=int, default=6969,
        help="Porta (padrão 6969)",
    )
    parser.add_argument(
